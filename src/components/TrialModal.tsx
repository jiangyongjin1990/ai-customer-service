...
export default function TrialModal({ isOpen, onClose }: TrialModalProps) {
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      ...
      // 对导航栏应用相同的补偿和隐藏处理
      const navbar = document.querySelector('#fixed-navbar-container');
      if (navbar) {
  ...
  return isOpen ? (
    <div className="fixed inset-0 z-[100000] overflow-y-auto flex items-center justify-center" style={{ zIndex: 100000 }}>
      {/* 背景遮罩 */}
      <div 
      ...
      <div className="px-4 text-center relative z-[100001] w-full max-w-4xl">
        {/* 模态窗内容 */}
        <div 
          className="inline-block w-full text-left align-middle bg-white rounded-[20px] shadow-[0_8px_50px_rgba(0,0,0,0.15)] overflow-hidden transition-all duration-300 transform"
          onClick={(e) => e.stopPropagation()}
          style={{ 
            position: 'relative', 
            zIndex: 100002,
            maxHeight: '90vh',
            animation: isOpen ? 'modal-fade-in 0.3s ease-out forwards' : 'none'
          }}
        >
