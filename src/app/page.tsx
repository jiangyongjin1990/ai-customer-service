...
const TenYearsBadge = () => (
   <span className="ml-1.5 px-1.5 py-0.5 bg-[rgb(var(--color-tranquil-blue))] text-white text-[9px] leading-tight rounded-full font-medium">10years</span>
);
    ...
    <main className="gradient-bg">
      ...
      >
        <div className="container mx-auto px-4">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-center mb-8 md:mb-12 text-gray-900">
            在线体验与<span className="gradient-text-2024">小嘉 AI</span> 对话
          </h2>
          <p className="text-sm sm:text-base text-gray-500 text-center mb-8 max-w-xl mx-auto">
            选择一个行业与场景，在对话框中输入任何你想问的问题
          </p>
          
          <div className="max-w-4xl mx-auto glass-card-intense glass-card-transition rounded-xl overflow-hidden flex flex-col md:flex-row border border-indigo-100/50 shadow-lg">
            {/* Left Panel - Enhanced for mobile */}
            <div className="md:w-1/3 p-3 md:p-4 border-b md:border-b-0 md:border-r border-gray-100/40 md:space-y-2 bg-white/10 overflow-x-auto">
              {/* Mobile Industry Selector */}
              <div className="flex md:hidden pb-2 overflow-x-auto scrollbar-hide mb-1">
                <div className="flex space-x-2 px-1">
                  {["电商零售", "金融保险", "教育培训", "旅游酒店", "医疗健康", "房产家居"].map((industry, i) => (
                    <button 
                      key={i} 
                      className={`whitespace-nowrap px-3 py-2 rounded-md cursor-pointer transition-all text-xs font-medium ${i === activeIndustry ? 'bg-gradient-to-r from-[#4e90cc] to-[#9478f0] text-white shadow-inner' : 'glass-card glass-hover hover:bg-white/40 text-gray-700'}`}
              ...
              <div className="hidden md:block space-y-2">
                {["电商零售", "金融保险", "教育培训", "旅游酒店", "医疗健康", "房产家居"].map((industry, i) => (
                  <button 
                    key={i} 
                    className={`w-full text-left p-3 rounded-md cursor-pointer transition-all text-sm font-medium ${i === activeIndustry ? 'bg-gradient-to-r from-[#4e90cc] to-[#9478f0] text-white shadow-inner' : 'glass-card glass-hover hover:bg-white/40 text-gray-700'}`}
                    onClick={() => setActiveIndustry(i)}
                  >
                    {industry}
                  </button>
            ...
            {/* Right Panel */}
            <div className="md:w-2/3 p-4 md:p-5 flex flex-col bg-white/30">
              <div className="flex-grow h-64 md:h-72 overflow-y-auto space-y-4 text-sm mb-4 pr-2 custom-scrollbar">
                {/* Sample Chat Messages */}
                <div className="flex justify-start">
                  <div className="glass-blue py-2 px-3.5 rounded-lg rounded-bl-none max-w-[85%] text-gray-800">
                    你好！我是小嘉 AI 智能助手，很高兴为您服务。有什么可以帮您的吗？
                  </div>
                </div>
                <div className="flex justify-end">
                  <div className="bg-gradient-to-r from-[#4e90cc] via-[#7a85e0] to-[#9478f0] text-white py-2 px-3.5 rounded-lg rounded-br-none max-w-[85%]">
                    我想了解一下你们的退货政策。
                  </div>
                </div>
                <div className="flex justify-start">
                  <div className="glass-blue py-2 px-3.5 rounded-lg rounded-bl-none max-w-[85%] text-gray-800">
                    我们支持7天无理由退货，商品需保持完好。质量问题30天内可退换。您可以在官网帮助中心查看详细说明。
                  </div>
                </div>
              </div>
              {/* Input Area - Enhanced for mobile */}
              <div className="flex items-center gap-2 mt-auto border-t border-gray-200/30 pt-4">
                <input 
                  type="text" 
                  placeholder="输入您的问题..." 
                  className="flex-1 px-4 py-2.5 sm:py-2 text-sm glass-card border-0 rounded-full focus:outline-none focus:ring-2 focus:ring-[#4e90cc] bg-white/40"
                />
                <button className="button-transition p-2.5 bg-gradient-to-r from-[#4e90cc] via-[#7a85e0] to-[#9478f0] text-white rounded-full hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                     <path d="M10.894 2.553a1 1 0 00-1.788 0l-7 14a1 1 0 001.169 1.409l5-1.429A1 1 0 009 16.571V11.69l-4.585 1.294a1 1 0 00-.737 1.536l6.21 10.414a1 1 0 001.67 0l6.21-10.414a1 1 0 00-.736-1.536L11 11.691V16.57a1 1 0 00.725.962l5 1.428a1 1 0 001.17-1.408l-7-14z" />
                  </svg>
                </button>
              </div>
            </div>
      ...
      >
        <div className="container mx-auto px-4 text-center relative z-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-y-6 gap-x-4 md:gap-x-6 max-w-4xl mx-auto">
            <motion.div 
              className="glass-blue glass-card-transition p-3 rounded-xl border border-blue-200/50 shadow-md hover:shadow-lg transition-shadow duration-300"
              initial={{ opacity: 0, y: 20 }}
            ...
            <motion.div 
              className="glass-purple glass-card-transition p-3 rounded-xl border border-purple-200/50 shadow-md hover:shadow-lg transition-shadow duration-300"
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            <motion.div 
              className="glass-teal glass-card-transition p-3 rounded-xl border border-teal-200/50 shadow-md hover:shadow-lg transition-shadow duration-300"
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5, delay: 0.3 }}
              viewport={{ once: true }}
            ...
            <motion.div 
              className="glass-amber glass-card-transition p-3 rounded-xl border border-amber-200/50 shadow-md hover:shadow-lg transition-shadow duration-300"
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5, delay: 0.4 }}
              viewport={{ once: true }}
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-8 md:mb-12 max-w-5xl mx-auto">
            {[
              ...
              <motion.div 
                key={index} 
                className={`glass-${item.color} glass-card-transition p-3 sm:p-4 rounded-lg shadow-md hover:shadow-lg hover:scale-105 flex items-center`}
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4 sm:gap-6 max-w-6xl mx-auto text-center">
             {[
               ...
               <motion.div 
                 key={index} 
                 className="p-3 sm:p-4 hover:bg-white/60 rounded-lg transition-colors duration-200 cursor-default"
      ...
      >
        <div className="container mx-auto px-4">
          ...
          {/* 特性卡片区域 - 使用浅蓝色背景 */}
          <div className="bg-transparent grid md:grid-cols-2 gap-6 md:gap-8 mb-16 max-w-5xl mx-auto">
            {/* Card 1: 全渠道在线客服 - 左文右图 */}
            <motion.div 
                ...
                <div className="p-6 flex flex-col justify-between">
                  <div>
                    <div className="flex items-center mb-3">
                      <h4 className="text-xl font-bold text-gray-900 flex items-center">全渠道在线客服 <TenYearsBadge/></h4>
                    </div>
                    <p className="text-gray-600 mb-4 text-sm">一个平台全部渠道，精准转化，高效服务</p>
                    <ul className="space-y-2 text-gray-700 text-sm mb-4">
                      <li className="flex items-start"><TickIcon className="mt-1 text-black"/>全部渠道，全部消息类型</li>
                      <li className="flex items-start"><TickIcon className="mt-1 text-black"/>可集成 AI，高效获线</li>
                      <li className="flex items-start"><TickIcon className="mt-1 text-black"/>智能分配，团队协同</li>
                      <li className="flex items-start"><TickIcon className="mt-1 text-black"/>多维数据，实时监控</li>
                    </ul>
                  </div>
                  <Link href="/products/customer-service" className="text-blue-600 hover:text-blue-800 font-medium text-sm group inline-flex items-center mt-2">
                    立即咨询 <span className="transition-transform duration-200 group-hover:translate-x-1 ml-1 text-base">→</span>
                  </Link>
                </div>
                {/* 替换第四张图片 - 客服系统界面 */}
          ...
          <Image
                      src="/images/customer-service-interface.png"
                      alt="全渠道在线客服功能示意图"
                      width={400} 
                      height={300}
                      className="object-contain w-full transform transition-transform duration-700 hover:scale-105" 
                    />
                  </div>
                </div>
              </div>
            ...
            >
              <div className="flex flex-col">
                ...
                <div className="p-6 flex flex-col justify-between">
                  <div>
                    <h4 className="text-xl font-bold mb-3 text-gray-900">智能客服机器人</h4>
                    <p className="text-gray-600 mb-4 text-sm">独立解决 90% 以上的常见问题</p>
                    <ul className="space-y-2 text-gray-700 text-sm mb-4">
                      <li className="flex items-start"><TickIcon className="mt-1 text-black"/>7x24 小时自动化</li>
                      <li className="flex items-start"><TickIcon className="mt-1 text-black"/>意图识别，自动标记</li>
                      <li className="flex items-start"><TickIcon className="mt-1 text-black"/>辅助人工，人机协同</li>
                      <li className="flex items-start"><TickIcon className="mt-1 text-black"/>多轮对话，一问多答</li>
                    </ul>
                  </div>
                  <Link href="/products/chatbot" className="text-blue-600 hover:text-blue-800 font-medium text-sm group inline-flex items-center mt-2">
                    立即咨询 <span className="transition-transform duration-200 group-hover:translate-x-1 ml-1 text-base">→</span>
                  </Link>
                </div>
                {/* 替换第三张图片 - 多渠道平台图标 */}
                <div className="px-6 pb-6">
                  <div className="overflow-hidden rounded-md border border-gray-100">
          <Image
                      src="/images/multi-channel-icons.png"
                      alt="多渠道平台集成"
                      width={400} 
                      height={300}
                      className="object-contain w-full transform transition-transform duration-700 hover:scale-105" 
          ...
          <div className="grid md:grid-cols-2 gap-8 md:gap-12 items-center max-w-5xl mx-auto">
             ...
             > 
               <motion.h4 
                 className="text-xl md:text-2xl font-bold mb-4 text-gray-900 flex items-center"
                 variants={itemVariants}
               >
                 大模型电话客服 <AiAgentBadge/>
               </motion.h4>
               <motion.p 
                 className="text-gray-600 mb-5 text-sm md:text-base leading-relaxed"
                 variants={itemVariants}
               >
                 还原真人快速响应，7x24h 热情服务。真人声音复刻，实时意图分析，超低延时，更低成本，提升语音服务效率与客户体验。
               </motion.p>
               <motion.ul 
                 className="space-y-2.5 text-gray-700 mb-6 text-sm md:text-base"
                 variants={containerVariants}
               >
                  <motion.li variants={itemVariants} className="flex items-center"><TickIcon className="text-black"/>AI 自主接待</motion.li>
                  <motion.li variants={itemVariants} className="flex items-center"><TickIcon className="text-black"/>情绪检测，智能打断</motion.li>
                  <motion.li variants={itemVariants} className="flex items-center"><TickIcon className="text-black"/>快速创建，灵活应用场景</motion.li>
                  <motion.li variants={itemVariants} className="flex items-center"><TickIcon className="text-black"/>多音色免费测试</motion.li>
               </motion.ul>
               <motion.div variants={itemVariants}>
                 <Link href="/products/call-center" className="text-blue-600 hover:text-blue-800 font-medium text-sm md:text-base group inline-flex items-center">
                    立即咨询 <span className="transition-transform duration-200 group-hover:translate-x-1.5 ml-1.5 text-lg">→</span>
                 </Link>
               </motion.div>
            </motion.div>
            ...
            >
              <div className="aspect-w-4 aspect-h-3">
          <Image
                  src="/images/product-documents.png" 
                  alt="产品资料文档展示" 
                  width={500} 
                  height={400}
                  className="object-cover rounded-md transform transition-transform duration-700 hover:scale-105" 
      ...
      >
        <div className="container mx-auto px-4">
          <h2 className="text-2xl md:text-3xl font-bold text-center mb-3 md:mb-4 text-gray-900">为什么选择<span className="bg-gradient-to-r from-cyan-500 via-blue-600 to-purple-600 bg-clip-text text-transparent">小嘉</span></h2>
