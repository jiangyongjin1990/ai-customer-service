...
export async function POST(req: NextRequest) {
  console.log('[DeepSeek-Test] 收到请求');
  
  try {
    // 解析请求
    const body = await req.json();
    console.log('[DeepSeek-Test] 请求体:', body);
    
    const userMessage = body.message;
    console.log('[DeepSeek-Test] 用户消息:', userMessage);

    if (!userMessage) {
      console.log('[DeepSeek-Test] 错误: 缺少消息字段');
    ...
    try {
      // 调用硅基流动 DeepSeek API
      console.log('[DeepSeek-Test] 调用 DeepSeek API');
      const API_KEY = 'sk-vtfsmocgfmxscxwfwikrvntupmykphbkolybsvzvwydubsiv';
      const API_URL = 'https://api.siliconflow.cn/v1/chat/completions';
      
      console.log('[DeepSeek-Test] 使用API URL:', API_URL);
...
请用最自然的方式与用户交流，让他们感觉是在和一个有经验、有个性的真人客服交流。`
      ...
      console.log('[DeepSeek-Test] 请求参数:', JSON.stringify({
        model: payload.model,
      ...
      console.log('[DeepSeek-Test] DeepSeek 响应状态:', response.status);
      
      if (!response.ok) {
        const errorText = await response.text();
        console.error('[DeepSeek-Test] DeepSeek API 响应错误:', response.status, errorText);
      ...
      console.log('[DeepSeek-Test] DeepSeek 响应成功, ID:', data.id);
      
      // 提取回复内容
      const reply = data.choices?.[0]?.message?.content || '抱歉，无法获取回复';
      console.log('[DeepSeek-Test] DeepSeek 回复 (部分):', reply.substring(0, 50) + '...');
      ...
      console.error('[DeepSeek-Test] DeepSeek API调用失败:', apiError?.message || '未知错误');
    ...
    console.error('[DeepSeek-Test] 处理请求出错:', error?.message || '未知错误');
    return NextResponse.json({ 
      error: '服务器内部错误',
...
function cleanupExpiredDialogues() {
  ...
  dialogueCache.forEach((dialogue, userId) => {
    if (now - dialogue.lastUpdateTime > EXPIRATION_TIME) {
      ...
      console.log(`[DeepSeek-Test] 清理过期对话: ${userId}`);
    }
  });
}
